...
class CheckoutNotPaidManagement implements CheckoutNotPaidManagementInterface
{
    ...
    public function postCheckoutNotPaid($incrementId)
    {
        ...
        if ($order = $orderArray['order']) {
            ...
            $return['message'] = $this->aplazoHelper->getCancelMessage();
            if($this->aplazoHelper->getEnableRecoverCart()){
                ...
                try {
                    ...
                    if($canRecoverQuote){
                        ...
                        $return['quoteId'] = $newQuote->getId();
                    } else {
                        $return['messageError'] = 'No se pudo recuperar el carrito';
                    }
                } catch (\Exception $e) {
                    $return['messageError'] = 'No se pudo recuperar el carrito. Mensaje de error ' .$e->getMessage();
